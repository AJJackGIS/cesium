...
function RectangularSensorPrimitive(options) {
   ...
   */
  this.scanPlaneMode = options.scanPlaneMode ?? "horizontal";
...
function computeUnitPosition(primitive, xHalfAngle, yHalfAngle) {
  ...
  const zoy = [];
  for (i = 0; i < slice; i++) {
    phi = (2 * maxY * i) / (slice - 1) - maxY;
    zoy.push(new Cartesian3(0, Math.sin(phi), Math.cos(phi)));
  }
  //zox面单位圆
  const zox = [];
  for (i = 0; i < slice; i++) {
    phi = (2 * maxX * i) / (slice - 1) - maxX;
    zox.push(new Cartesian3(Math.sin(phi), 0, Math.cos(phi)));
  }
...
function createVertexArray(primitive, frameState) {
  ...
  if (primitive.showScanPlane) {
    if (primitive.scanPlaneMode === "horizontal") {
      unitScanPlanePositions = computeUnitPosition(
        primitive,
